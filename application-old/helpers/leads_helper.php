...
function render_customer_status_select($selected = '', $lang_key = '', $name = 'status',  $exclude_default = false)
{
    // Static statuses
    $statuses = [
        ['id' => '1', 'name' => 'Under EPR'],
        ['id' => '2', 'name' => 'Under Installation'],
        ['id' => '3', 'name' => 'Under Submission'],
        ['id' => '4', 'name' => 'Completed']
    ];

    // If excluding the default status is requested, remove the first status
    if ($exclude_default == true) {
        unset($statuses[0]);
    }    
     $select_attrs['id'] =  'selectcustomer'; // default id to the name of the select
     $select_attrs['data-live-search'] =  'false'; // default id to the name of the select
     
    // Return a regular select element with the provided statuses
    return render_select($name, $statuses, ['id', 'name'], $lang_key, $selected, $select_attrs);
}
...
function get_lead_name_by_id($lead_id)
{
    $CI = & get_instance();
    $CI->db->where('id', $lead_id);
    $lead = $CI->db->get(db_prefix() . 'leads')->row();  
...
function get_source_name_by_id($source_id)
{
    ...
    $lead = $CI->db->get(db_prefix() . 'leads_sources')->row();  
