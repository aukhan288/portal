...
function get_required_fields_for_registration()
{
    ...
    return [
        'contact' => [
            ...
            'contact_lastname' => ['label' => _l('clients_lastname'), 'is_required' => false, 'disabled' => false],
...
function count_non_empty_custom_fields($leadid) {
    $custom_fields = get_custom_fields('leads');
    $count = 0;

    foreach ($custom_fields as $field) {
        $value = get_custom_field_value($leadid , $field['id'], 'leads');  
        if ($value != '') {
            $count++;
        }
    }
...
if (!function_exists('get_surveys_by_customer_id')) {
    /**
     * Retrieve all surveys by user ID
     *
     * @param int $customer_id
     * @return array
     */
    function get_surveys_by_customer_id($customer_id)
    {
        ...
        // Query the database to retrieve all surveys for the given user ID
        $CI->db->where('customer_id', $customer_id);
        $query = $CI->db->get('tblclient_survey_statuses ');

        // Return the result as an array of objects
        return $query->result();
    }
}
if (!function_exists('get_workflows_by_customer_id')) {
    /**
     * Retrieve all workflow status by user ID
     *
     * @param int $customer_id
     * @return array
     */
    function get_workflows_by_customer_id($customer_id)
    {
        // Get an instance of CodeIgniter
        $CI =& get_instance();
        ...
        // Query the database to retrieve all workflow status for the given user ID
        $CI->db->where('customer_id', $customer_id);
        $query = $CI->db->get('tblclient_worflow_statuses');

        // Return the result as an array of objects
        return $query->result();
    }
}
if (!function_exists('get_dwps_by_customer_id')) {
    /**
     * Retrieve all workflow status by user ID
     *
     * @param int $customer_id
     * @return array
     */
    function get_dwps_by_customer_id($customer_id)
    {
        // Get an instance of CodeIgniter
        $CI =& get_instance();
        ...
        // Query the database to retrieve all workflow status for the given user ID
        $CI->db->where('customer_id', $customer_id);
        $query = $CI->db->get('tblclient_dwp_statuses');
